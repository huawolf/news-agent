...
<body><style>.source-toolbar{display:grid;grid-template-columns:minmax(240px,1fr) minmax(180px,auto);gap:8px}.source-toolbar select{width:100%}.source-item{grid-template-columns:minmax(160px,1fr) 2fr auto auto;align-items:start}.source-results .source-item{grid-template-columns:minmax(160px,1fr) 2fr auto}.source-remove{padding:5px 8px;font-size:12px}@media(max-width:720px){.source-toolbar,.source-item,.source-results .source-item{grid-template-columns:1fr}}</style><main><header><div><h1>News Agent</h1><p data-t=\"subtitle\">Local news collection, delivery, and agent control.</p></div><div class=\"header-actions\"><span id=\"status\" class=\"status\"></span><select id=\"language\" onchange=\"setLanguage(this.value)\"><option value=\"en\">English</option><option value=\"zh\">中文</option></select></div></header>
...
const sourceCategories=['ai','developer_open_source','product_startup','business_investment','technology_policy','other'];let lang=localStorage.getItem('news-agent-language')||'en',sourceData={rss:[],integrations:[]},currentRunJobId=null;const $=id=>document.getElementById(id);const t=k=>words[lang][k]||k;const categoryLabel=id=>t('category_'+(id||'other'));
